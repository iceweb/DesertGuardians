...
/**
 * NextWavePanel displays upcoming wave creep types as icons
 * in a themed info box at the bottom-left of the screen.
 * Features:
 * - Realistic creep icons matching in-game visuals
 * - Golden glow effect for boss waves
 * - Hover tooltips showing creep abilities
 */
export class NextWavePanel {
  ...
  /**
   * Show the panel with next wave info
   */
  show(waveNumber: number, creepTypes: Array<{ type: string; description: string }>, waveType?: WaveType): void {
    ...
    // Create boss glow effect (if boss wave)
    if (isBossWave) {
  ...
  /**
   * Create an icon with hover tooltip functionality
   */
  ...
  /**
   * Format creep type name for display
   */
  ...
  /**
   * Update panel position (if needed for responsive layout)
   */
  updatePosition(): void {
